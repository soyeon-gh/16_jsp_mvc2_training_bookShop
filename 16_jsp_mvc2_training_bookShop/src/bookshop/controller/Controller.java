	...
	protected void requestPro(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		try {
	        ...
	        if (command.indexOf(request.getContextPath()) == 0) { 
	        																		// /index.do
	        																		// 2. *.do 앞의 url을 제거하여 map의 키 값과 같은 url형태로 파싱한다.	
	           command = command.substring(request.getContextPath().length());		// /16_jsp_mvc2_training_bookShop 
	       																			...
	       																			// bookshop.command._11_ShopMain@4cb3b4f0
	        com = (CommandAction)commandMap.get(command);							// 3. 2에서 얻은 url로 해당 클래스의 객체를 생성한다. 
	        																		//System.out.println("com : " + com);									
	        																		// 4. 3에서 얻은 클래스의 requestPro메서드를 호출하여 
	        																		//클래스의 url의 return값인 jsp파일명을 얻어온다.   /11_shopMain.jsp
